...
@section('content')
    <div class="container mx-auto py-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Edit Attribute Produk</h1>
        ...
        <form action="{{ route('admin.product.attribute.update', $attribute->id) }}" method="POST"
            ...
            <div>
                ...
                <select name="product_id" id="product_id"
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->id }}" {{ $product->id == $attribute->product_id ? 'selected' : '' }}>
                            {{ $product->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="size" id="size" value="{{ old('size', $attribute->size) }}"
                    class="block w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="color" id="color" value="{{ old('color', $attribute->color) }}"
                    class="block w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    required>
            </div>
